...
const NAV_RENDER_BUILD = "nav-v6-profile-drawer";
...
function storedUser(){
  try{return JSON.parse(localStorage.getItem("evaraos-user")||"null")||{}}catch{return{}}
}
...
function quickLinks(){
  const items=[["Home","index.html","⌂"],["Dashboard","dashboard.html","◫"],["Leads","leads.html","◎"],["Jobs","jobs.html","◇"],["Settings","settings.html","⚙"]];
...
function accountLinks(authed){
  ...
  const items=[["Account","settings/account.html","◉"],["Appearance","settings/appearance.html","◐"],["Alerts","notifications_center.html","◌"],["Workspace","settings/workspace.html","◈"]];
...
function appSections(role){
  ...
  return CATEGORY_ORDER.map((category)=>{
    const apps=groups[category]||[];if(!apps.length)return"";
...
export function renderNav(){
  ...
  const alertsHref=buildHref("notifications_center.html");
  mount.innerHTML=`<div class="eva-nav-layer" data-render-build="${NAV_RENDER_BUILD}"><header class="eva-nav-shell" id="evaNavShell"><div class="eva-nav-pill glass-shell" id="evaNavPill"><div class="eva-menu-zone" id="evaMenuZone"><button class="eva-profile-trigger" type="button" id="evaMenuBtn" aria-expanded="false" aria-label="Open profile menu">${avatar}</button></div><a class="eva-brand" id="evaBrandBlock" data-home-link="${buildHref("index.html")}" href="${buildHref("index.html")}" aria-label="Evaraos home"><span class="eva-brand-wordmark">Evaraos</span></a><a class="eva-top-alert" href="${alertsHref}" data-menu-link="${alertsHref}" aria-label="Notifications"><span aria-hidden="true">◌</span></a></div></header><div class="eva-backdrop" id="evaBackdrop"></div><aside class="eva-menu-panel" id="evaMenuPanel" aria-label="Evaraos navigation drawer"><div class="eva-drawer-profile"><div class="eva-drawer-avatar">${avatar}</div><div><strong>${clean(profile.name)}</strong><span>${clean(normalizeRole(groups.role))}</span></div><button type="button" id="evaMenuCloseBtn" aria-label="Close menu">×</button></div><nav class="eva-menu-apps" id="evaLinks" data-nav-role="${clean(normalizeRole(groups.role))}">${quickLinks()}${accountLinks(groups.authed)}${groups.authed?appSections(groups.role):""}</nav>${aiPrompt(groups.authed)}</aside></div>`;
  return true;
}

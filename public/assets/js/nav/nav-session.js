...
export async function startVerifiedPublicHomeSession() {
  ...
  try {
    ...
    firebase.onAuthStateChanged(firebase.auth, (user) => {
      ...
      // Revoke the old shell immediately while the next profile is verified.
      publishPublicHomeSession({ authenticated: false, lifecycle: user ? "verification_required" : "guest" });
...
export async function rebindNavAfterRender() {
  ...
  if (close && close.dataset.publicGuestCloseBound !== "true") {
    ...
    close.addEventListener("click", () => menu.closeMenu());
  }
...
export function refreshNav() {
  refreshQueue = refreshQueue.then(performNavRefresh, performNavRefresh);
  return refreshQueue;
}

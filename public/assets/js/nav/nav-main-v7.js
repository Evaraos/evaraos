const NAV_BUILD = 'nav-v61-role-authority';
...
async function loadCore() {
  const [config, utils, renderer] = await Promise.all([
    ...
    import(`./nav-utils-v2.js?v=${NAV_BUILD}`),
    import(`./nav-render-v2.js?v=${NAV_BUILD}`)
  ]);
...
function keepVisible() {
  [document.getElementById('evaNavShell'), document.getElementById('evaMenuBtn'), document.getElementById('globalNotificationsBell')]
    .filter(Boolean)
    .forEach((node) => {
...
async function bindCoreSystems() {
  ...
  try {
    ...
    console.warn('Nav theme refresh failed:', error);
  }
}

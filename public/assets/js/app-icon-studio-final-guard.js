(() => {
  ...
  const ASSET_ICON = '/assets/brand/evaraos-app-icon.png?v=brand-icon-final-4';
  const B64_ICON = '/assets/brand/evaraos-icon-transparent-128.png.b64?v=brand-icon-final-4';
  const OFFICIAL_KEY = 'evaraos-official-app-icon-v2';
  const USER_ICON_KEY = 'evaraos-user-app-icon-v1';
  const LEGACY_CUSTOM_KEYS = ['evaraos-custom-app-icon-v5','evaraos-custom-app-icon-v4','evaraos-custom-app-icon-v3','evaraos-custom-icon-v1','evaraos-custom-app-icon-v2'];
  const OWNER_ROLES = new Set(['owner','super_admin','admin']);

  function readDataUrl(key) { try { const value = localStorage.getItem(key); return value && value.startsWith('data:image/') ? value : ''; } catch { return ''; } }
  function userIcon() { return readDataUrl(USER_ICON_KEY) || LEGACY_CUSTOM_KEYS.map(readDataUrl).find(Boolean) || ''; }
  function repoIcon() { return readDataUrl(OFFICIAL_KEY) || ASSET_ICON; }
  function activeIcon() { return userIcon() || repoIcon(); }
  ...
  function ensureRuntimeStyle() {
    ...
    style.textContent = `
      ...
      .app-icon-logo-img,.app-icon-preview.is-backgroundless .app-icon-logo-img,.app-icon-title-mark.is-backgroundless .app-icon-logo-img{display:block!important;opacity:1!important;visibility:visible!important;width:92%!important;height:92%!important;max-width:92%!important;max-height:92%!important;object-fit:contain!important;object-position:center!important;filter:none!important;mix-blend-mode:normal!important;position:relative!important;z-index:10!important;}
  ...
  function forceImages(src = activeIcon()) {
    ...
    document.querySelectorAll('[data-brand-title-mark],[data-current-icon-preview],[data-device-preview],[data-device-preview-light],.app-icon-preview.is-backgroundless').forEach((node) => {
      ...
      if (!img) { img = document.createElement('img'); img.className = 'app-icon-logo-img'; img.alt = ''; node.replaceChildren(img); }
      img.src = src;
  ...
  function clearUserIcon() { try { localStorage.removeItem(USER_ICON_KEY); LEGACY_CUSTOM_KEYS.forEach((key) => localStorage.removeItem(key)); } catch {} }
  function saveUserIcon(src) { try { localStorage.setItem(USER_ICON_KEY, src); } catch {} forceImages(src); toast('Custom icon uploaded'); }
  function saveOfficialIcon(src) { try { localStorage.setItem(OFFICIAL_KEY, src); } catch {} clearUserIcon(); forceImages(src); toast('Official default icon updated'); }

  function fileToDataUrl(file, callback) {
    if (!file || !/^image\/(png|jpeg|webp)$/.test(file.type)) { toast('Use PNG, JPG, or WEBP'); return; }
    const reader = new FileReader();
    reader.onload = () => { const src = String(reader.result || ''); src.startsWith('data:image/') ? callback(src) : toast('Upload failed'); };

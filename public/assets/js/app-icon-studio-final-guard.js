(() => {
  ...
  const ASSET_ICON = '/assets/brand/evaraos-app-icon.png?v=brand-icon-final-6';
  ...
  function readDataUrl(key) { try { const value = localStorage.getItem(key); return value && /^data:image\/(png|jpeg|webp);base64,/.test(value) ? value : ''; } catch { return ''; } }
  function writeDataUrl(key, src) { if (!src || !/^data:image\/(png|jpeg|webp);base64,/.test(src)) return false; try { localStorage.setItem(key, src); return true; } catch { return false; } }
  ...
  function findEmbeddedOfficialIcon() {
    const nodes = Array.from(document.querySelectorAll('img.app-icon-logo-img'));
    for (const img of nodes) {
      const src = img.getAttribute('src') || img.src || '';
      if (/^data:image\/(png|jpeg|webp);base64,/.test(src) && !src.includes('UEsDB')) return src;
    }
    return '';
  }
  ...
  function patchImage(img, src) { if (!img) return; img.src = src; img.onerror = () => { img.onerror = null; img.src = officialIcon(); }; img.style.setProperty('display','block','important'); img.style.setProperty('opacity','1','important'); img.style.setProperty('visibility','visible','important'); img.style.setProperty('object-fit','contain','important'); }
  ...
  function fileToDataUrl(file, callback) { if (!file || !/^image\/(png|jpeg|webp)$/.test(file.type)) { toast('Use PNG, JPG, or WEBP'); return; } const reader = new FileReader(); reader.onload = () => { const src = String(reader.result || ''); /^data:image\/(png|jpeg|webp);base64,/.test(src) ? callback(src) : toast('Upload failed'); }; reader.onerror = () => toast('Upload failed'); reader.readAsDataURL(file); }
  ...
  async function boot() { clearBadLegacy(); ensureRuntimeStyle(); ensureOwnerPanel(); ensureOwnerFab(); setTimeout(bindUpload, 60); bindRestore(); bindOwnerAccess(); const src = await hydrateOfficialIcon(false); forceImages(userIcon() || src); setTimeout(() => { bindUpload(); hydrateOfficialIcon(false).then(() => forceImages()); }, 300); setTimeout(() => forceImages(), 1200); const grid = document.querySelector('[data-app-icon-grid]'); if (grid) new MutationObserver(() => requestAnimationFrame(() => forceImages())).observe(grid, { childList: true, subtree: true }); }

  window.addEventListener('pageshow', () => setTimeout(() => hydrateOfficialIcon(false).then((src) => forceImages(userIcon() || src)), 60));

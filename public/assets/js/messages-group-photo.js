...
function isPlatformUser() {
  ...
  return role === "owner"
    || role === "super_admin"
    || (role === "admin" && profile.platformAccess === true);
}
...
async function findRegistry(conversationId) {
  ...
  const document = doc(db, "channels", REGISTRY, "messages", conversationId);
  const snapshot = await getDoc(document);
  if (!snapshot.exists()) return null;
  const data = snapshot.data() || {};
  if (data.groupId !== conversationId) return null;
  return { document, data: { registryDocId: document.id, ...data } };
}

async function resolveActiveRegistry() {
  if (!activeConversationId) return null;
  return findRegistry(activeConversationId);
}

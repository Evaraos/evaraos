import {
  auth, db, onAuthStateChanged, collection, doc, getDoc, getDocs, addDoc, setDoc, updateDoc,
...
async function loadConversations(){
  const dynamic=[];
  try{const snap=await getDocs(collection(db,"channels"));snap.docs.forEach(d=>dynamic.push({id:d.id,...d.data()}))}catch(error){console.warn("Channel metadata unavailable",error)}
  const builtin=BUILTIN.map(item=>({...item,builtin:true,type:"role",imageUrl:"",memberUids:[],adminUids:[]}));
  const merged=[...builtin,...dynamic.filter(item=>!BUILTIN.some(base=>base.id===item.id))].filter(canSee);
...
function groupForm(conversation=null){
  ...
  $("groupSheetTitle").textContent=conversation?"Edit group":"New group";$("groupName").value=conversation?.name||"";$("groupImageUrl").value=conversation?.imageUrl||"";$("groupImagePreview").innerHTML=conversation?.imageUrl?`<img src="${esc(conversation.imageUrl)}" alt="Group"/>`:`<span>${esc(initials(conversation?.name||"New Group"))}</span>`;$("groupConversationId").value=conversation?.id||"";
...
async function saveGroup(event){
  event.preventDefault();const id=$("groupConversationId").value||`group-${crypto.randomUUID()}`;const members=[state.user.uid,...[...$("groupMembers").querySelectorAll("input:checked")].map(input=>input.value)];const existing=state.conversations.find(item=>item.id===id);const adminUids=existing?.adminUids?.length?existing.adminUids:[state.user.uid];
  const payload={name:$("groupName").value.trim(),imageUrl:$("groupImageUrl").value.trim(),description:"Group conversation",type:"group",memberUids:[...new Set(members)],adminUids,allowedRoles:[],companyId:state.profile.companyId||"",updatedAt:serverTimestamp()};
  if(!payload.name)throw new Error("Group name is required.");
  if(existing)await updateDoc(doc(db,"channels",id),payload);else await setDoc(doc(db,"channels",id),{...payload,createdBy:state.user.uid,createdAt:serverTimestamp()});
  $("groupSheet").hidden=true;await loadConversations();openConversation(id);
}
function bind(){
  ...
  $("newGroupBtn")?.addEventListener("click",()=>groupForm());$("editGroupBtn")?.addEventListener("click",()=>groupForm(state.active));$("closeGroupSheet")?.addEventListener("click",()=>$("groupSheet").hidden=true);$("groupForm")?.addEventListener("submit",e=>saveGroup(e).catch(error=>alert(error.message||"Unable to save group")));

(function(){
  ...
  const DEFAULT_EXPERIENCE=Object.freeze({
    ...
    loaders:{
      welcome:{enabled:true,eyebrow:'EVARAOS',title:'Welcome to Evaraos',subtitle:'Preparing your operating system.',minimumMs:450},
      page:{enabled:true,label:'Loading EvaraOS',delayMs:20},
      resume:{enabled:false,title:'Welcome back to Evaraos',subtitle:'Refreshing your workspace.',minimumAwayMs:45000}
    }
  });
  let experience=readExperienceCache();
  let timer=null,forceTimer=null,watchdogTimer=null,isTransitioning=false,welcomeStartedAt=0,activeWelcomeMinimum=450,hiddenAt=0;
  ...
  function normalizeExperience(raw={}){
    ...
    return{
      brand:{
        ...
        alt:text(raw?.brand?.alt,defaults.brand.alt,120)
      },
      loaderTheme:{
      ...
      loaders:{
        welcome:{
          ...
          minimumMs:integer(raw?.loaders?.welcome?.minimumMs,defaults.loaders.welcome.minimumMs,250,2000)
        },
        page:{
          ...
          delayMs:integer(raw?.loaders?.page?.delayMs,defaults.loaders.page.delayMs,0,250)
        },
        resume:{
  ...
  function applyBrand(){
    ...
    const escaped=mark.replaceAll('"','%22');
    document.documentElement.style.setProperty('--evaraos-brand-icon','url("'+escaped+'")');
    document.querySelectorAll('[data-evaraos-brand-icon]').forEach(node=>{
      node.style.setProperty('--evaraos-brand-icon','url("'+escaped+'")');
      node.style.backgroundImage='url("'+escaped+'")';
  ...
  function showFastLoader(){
    ...
    if(!experience.loaders.page.enabled)return false;
  ...
  function showWelcomeLoader(options={}){
    ...
    activeWelcomeMinimum=integer(options.minimumMs,variant==='welcome'?experience.loaders.welcome.minimumMs:Math.min(1600,experience.loaders.welcome.minimumMs),250,2000);
  ...
  function beginNavigationLoad(){
    ...
    if(experience.loaders.page.enabled)timer=setTimeout(showFastLoader,experience.loaders.page.delayMs);

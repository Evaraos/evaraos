(function(){
  ...
  const LEGACY_ICON_KEYS=[
    'evaraos-app-icon-snapshot-v2','evaraos-app-icon-selection-v2',
  ...
  function migrateLegacyBrandState(){
    try{
      ...
      Object.keys(localStorage).filter(key=>key.startsWith('evaraos-app-builder-v1:')).forEach(key=>localStorage.removeItem(key));
  ...
  function logoHtml(){return '<img class="evara-loader-mark" data-evaraos-brand-logo src="'+BRAND_LOGO_SRC+'" fetchpriority="high" decoding="async" alt="" aria-hidden="true">'}
  ...
  function applyBrand(){
    ...
    document.querySelectorAll('[data-evaraos-brand-logo],.sidebar-logo').forEach(node=>{
  ...
  function deactivate(node,immediate=false){
    if(!node)return;
    const finish=()=>{node.classList.remove('active','is-entering','is-exiting');node.setAttribute('aria-hidden','true');node.style.display='none'};
  ...
  function finishLoad(immediate=false){clearTimers();bootResolved=true;deactivate(document.getElementById(FAST_ID),immediate);hideWelcomeLoader(immediate);unlockVisualShell()}
  function criticalReady(){const navGate=!navRequired()||navReady||document.documentElement.dataset.evaraosNavReady==='true';const sessionGate=!routeRequiresSession()||sessionReady;return domReady&&navGate&&sessionGate&&!authPending()}
  function maybeCompleteBoot(immediate=false){if(bootResolved||isTransitioning||!criticalReady())return false;finishLoad(immediate);return true}
  function completeNavigationLoad(immediate=false){if(isTransitioning){finishLoad(immediate);return}maybeCompleteBoot(immediate)}
  ...
  function beginNavigationLoad(){
    ...
    isTransitioning=true;bootResolved=false;markInternalNavigation();clearTimers();ensure();
  ...
  function resetLeavingFrame(){clearTimers();transition()?.classList.remove('active');deactivate(document.getElementById(FAST_ID),true);document.body?.classList.remove('eva-page-leaving');isTransitioning=false}

  function health(){return{
    path:location.pathname,routeMode:routeMode(),launchMode:resolveLaunchMode(),appReady:document.body?.classList.contains('app-ready')===true,
    pending:appPending(),authPending:authPending(),domReady,windowLoaded,navReady:navReady||document.documentElement.dataset.evaraosNavReady==='true',
    sessionReady,criticalReady:criticalReady(),navBuild:document.documentElement.dataset.evaraosNavBuild||window.EVARAOS_NAV_BUILD||'',
    ...
    brandLogo:BRAND_LOGO_SRC,brandIcon:BRAND_ICON_SRC,brandMark:BRAND_ICON_SRC,appIcon:BRAND_ICON_SRC,brandVersion:BRAND_VERSION
  }}

  function shellWatchdog(){
    applyBrand();domReady=document.readyState!=='loading';windowLoaded=document.readyState==='complete';navReady=navReady||document.documentElement.dataset.evaraosNavReady==='true';
    ...
    if(root&&!root.querySelector('.eva-nav-layer')&&!document.documentElement.dataset.evaraosNavReady){import('/assets/js/nav.js?v=nav-v58-critical-shell').catch(error=>console.warn('Nav watchdog import failed:',error))}
  ...
  function handleVisibilityChange(){
    ...
    hiddenAt=0;bootResolved=false;showWelcomeLoader({title:'Welcome back to Evaraos',subtitle:'Refreshing your workspace.'});requestAnimationFrame(()=>maybeCompleteBoot(false));
  }

  function init(){
    migrateLegacyBrandState();ensureCriticalStyles();applyBrand();ensure();
    domReady=document.readyState!=='loading';windowLoaded=document.readyState==='complete';navReady=navReady||document.documentElement.dataset.evaraosNavReady==='true';
    ...
    if(preRenderedWelcome||mode==='launch')showWelcomeLoader();else if(document.body?.classList.contains('app-loading'))showFastLoader();
    ...
    window.EvaraBrandAssets=Object.freeze({version:BRAND_VERSION,logo:BRAND_LOGO_SRC,icon:BRAND_ICON_SRC,manifest:MANIFEST_SRC});
    window.EvaraLoader={beginNavigationLoad,completeNavigationLoad,showFastLoader,hideFastLoader:(immediate=false)=>deactivate(document.getElementById(FAST_ID),immediate),showFullLoader:showWelcomeLoader,hideFullLoader:hideWelcomeLoader,hideAllLoaders:completeNavigationLoad,markNavReady,markSessionReady,markAppReady:()=>maybeCompleteBoot(false),runShellWatchdog:shellWatchdog,getState:health,health};

    document.addEventListener('click',event=>{
      if(event.defaultPrevented||event.button!==0||event.metaKey||event.ctrlKey||event.shiftKey||event.altKey)return;
      const anchor=event.target.closest('a[href]');
      if(!shouldIntercept(anchor))return;
      event.preventDefault();beginNavigationLoad();setTimeout(()=>location.assign(anchor.href),80);
    });
    ...
    addEventListener('pageshow',event=>{
      windowLoaded=document.readyState==='complete';applyBrand();
      if(event.persisted&&(!routeRequiresSession()||sessionReady)&&!authPending()){finishLoad(true);return}
    ...
    if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>{domReady=true;applyBrand();maybeCompleteBoot(false)},{once:true});
    forceTimer=setTimeout(()=>{if(routeRequiresSession()&&(!sessionReady||authPending())){shellWatchdog();return}if(!maybeCompleteBoot(true))finishLoad(true)},FORCE_UNLOCK);

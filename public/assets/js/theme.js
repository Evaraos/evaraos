...
const THEME_STYLESHEET = "/assets/css/theme.css?v=ios27-liquid-v3";
const GLASS_SELECTOR = [
  ".glass-card", ".glass-shell", ".liquid-glass", ".aurora-card",
  "[class$='-card']", "[class*='-card ']", "[class$='-panel']", "[class*='-panel ']",
  ".dashboard-list-item", ".dashboard-feed-item", ".settings-block",
  ".eva-menu-panel", ".eva-bottom-nav", ".btn", "button", ".settings-chip",
  ".role-pill", ".role-pill-option", "[data-glass='card']", "[data-glass='interactive']"
].join(",");
...
export function normalizeMode(value) {
  return VALID_MODES.includes(value) ? value : DEFAULT_APPEARANCE.mode;
}

export function normalizeImageTheme(value) {
  return VALID_IMAGE_THEMES.includes(value) ? value : DEFAULT_APPEARANCE.imageTheme;
}
...
export function normalizeAppearance(value = {}) {
  return {
    ...
    glassTransparency: clamp(value.glassTransparency, .34, .76, DEFAULT_APPEARANCE.glassTransparency),
...
export function setImageTheme(imageTheme) {
  return saveAppearance({ mode: "image", imageTheme: normalizeImageTheme(imageTheme) });
}
...
function installOpticalResponse() {
  ...
  document.addEventListener("pointermove", event => {
    ...
    if (!target || target.matches(".appearance-source-backdrop")) return;

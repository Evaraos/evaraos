export const CANONICAL_ROLES = Object.freeze([
  'platform_admin','owner','admin','manager','sales','technician','cleaner','customer','vendor'
]);

const CANONICAL_ROLE_SET = new Set(CANONICAL_ROLES);
const ULTIMATE_ROLES = new Set(['platform_admin', 'owner']);

const ROLE_ALIASES = Object.freeze({
  platform_admin:'platform_admin',super_admin:'platform_admin',owner:'owner',admin:'admin',manager:'manager',
  operations_manager:'manager',operations_coordinator:'manager',field_manager:'manager',sales_manager:'manager',
  dispatcher:'manager',hr:'manager',hr_manager:'manager',customer_support_manager:'manager',customer_support:'manager',
  sales:'sales',sales_rep:'sales',lead_generator:'sales',technician:'technician',tech:'technician',lead_technician:'technician',
  cleaner:'cleaner',lead_cleaner:'cleaner',staff:'technician',field_staff:'technician',crew_lead:'technician',
  quality_control:'technician',customer:'customer',vendor:'vendor',lead_vendor:'vendor',service_vendor:'vendor',
  management_program:'vendor',organization:'vendor',organization_owner:'vendor',office_owner:'vendor',branch_owner:'vendor'
});
...
export const PUBLIC_PAGES = Object.freeze(new Set([
  '', 'index.html', 'login.html', 'signup.html', 'reset.html', 'staff_application.html'
]));
...
export const PAGE_POLICY = Object.freeze({
  ...
  'applications.html': ['platform_admin','owner','admin'],
...
export const FEATURE_POLICY = Object.freeze({
  studioView:['platform_admin','owner','admin'],studioEdit:['platform_admin','owner','admin'],
  blueprintPreview:['platform_admin','owner','admin'],blueprintPublish:['platform_admin','owner'],
  blueprintRollback:['platform_admin','owner'],manageUsers:['platform_admin','owner','admin','manager'],
  assignOwner:['platform_admin'],assignAdmin:['platform_admin','owner'],assignManager:['platform_admin','owner','admin'],
  assignStaff:['platform_admin','owner','admin'],manageApplications:['platform_admin','owner','admin'],
  viewFinance:['platform_admin','owner','admin','manager','vendor'],manageFinance:['platform_admin','owner','admin'],
  managePayroll:['platform_admin','owner','admin','manager'],manageJobs:['platform_admin','owner','admin','manager','vendor'],
  completeAssignedJobs:['technician','cleaner'],manageLeads:['platform_admin','owner','admin','manager','vendor','sales'],
  ...
  orderServices:['customer'],payInvoices:['customer'],viewOwnAccount:ALL_AUTHENTICATED,viewOwnMessages:ALL_AUTHENTICATED,
  manageVendorTeam:['platform_admin','owner','admin','vendor'],aiCommand:['platform_admin','owner','admin','manager'],
  brandManager:['platform_admin','owner','admin'],viewAudit:['platform_admin','owner','admin'],platformAdministration:['platform_admin']
});
...
export function isKnownRole(role = '') { return CANONICAL_ROLE_SET.has(normalizeAccessRole(role)); }
export function isPlatformAdmin(role = '') { return normalizeAccessRole(role) === 'platform_admin'; }
export function isPlatformOwner(role = '') { return ULTIMATE_ROLES.has(normalizeAccessRole(role)); }
export function isTenantExecutive(role = '') { return TENANT_EXECUTIVES.includes(normalizeAccessRole(role)); }
export function isTenantLeadership(role = '') { return TENANT_LEADERSHIP.includes(normalizeAccessRole(role)); }
export function isFieldRole(role = '') { return FIELD_ROLES.includes(normalizeAccessRole(role)); }
...
export function canUseFeature(feature = '', role = '') {
  ...
  return ULTIMATE_ROLES.has(normalizedRole) || allowed.includes(normalizedRole);
}
...
export function canAccessPageName(pageName = '', role = '') {
  ...
  if (Array.isArray(pathAllowed)) return Boolean(normalizedRole) && (ULTIMATE_ROLES.has(normalizedRole) || pathAllowed.includes(normalizedRole));
...
export function permissionsForRole(role = '') {
  ...
  if (ULTIMATE_ROLES.has(normalized)) return Object.keys(FEATURE_POLICY);
...
export function pagesForRole(role = '') {
  const normalized = normalizeAccessRole(role);
  if (!normalized) return [];
  const entries = [...Object.entries(PAGE_POLICY), ...Object.entries(PATH_POLICY)];
  if (ULTIMATE_ROLES.has(normalized)) return entries.map(([page]) => page);
  return entries.filter(([, allowed]) => allowed.includes(normalized)).map(([page]) => page);
}

export function accessDecision({ page = '', feature = '', role = '' } = {}) {
  const normalizedRole = normalizeAccessRole(role);
  if (feature) return Object.freeze({ allowed: canUseFeature(feature, normalizedRole), role: normalizedRole, resource: feature, type: 'feature' });
  return Object.freeze({ allowed: canAccessPageName(page, normalizedRole), role: normalizedRole, resource: page, type: 'page' });
}

...
function renewIds(root){root.querySelectorAll?.(EDITABLE_SELECTOR).forEach(node=>{node.dataset.nodeId=uid();node.classList.remove('studio-canvas-selected')});root.dataset.nodeId=uid();root.classList.remove('studio-canvas-selected');return root}
function snapshot(){
  if(!state.canvas)return'';
  const clone=state.canvas.cloneNode(true);
  clone.querySelectorAll('.studio-canvas-selected').forEach(node=>node.classList.remove('studio-canvas-selected'));
...
function restore(html){
  ...
  getNodes().forEach(node=>{nodeId(node);node.classList.remove('studio-canvas-selected')});
...
function beginDrag(event,node){
  ...
  if(!state.selected.length||state.selected.every(isLocked))return;
  const starts=state.selected.filter(item=>!isLocked(item)).map(item=>({node:item,left:px(item.style.left),top:px(item.style.top)}));
  state.drag={pointerId:event.pointerId,x:event.clientX,y:event.clientY,starts};starts.forEach(({node:item})=>ensurePositioned(item));document.body.classList.add('studio-is-transforming');event.target.setPointerCapture?.(event.pointerId);event.preventDefault();
}
function moveDrag(event){if(!state.drag||event.pointerId!==state.drag.pointerId)return;const dx=event.clientX-state.drag.x,dy=event.clientY-state.drag.y;state.drag.starts.forEach(({node,left,top})=>{const next=snapPosition(node,left+dx,top+dy);node.style.left=`${Math.round(next.left)}px`;node.style.top=`${Math.round(next.top)}px`});positionTransformBox();emit('evara:studio-transform-preview',{type:'move',nodes:state.drag.starts.map(item=>item.node)})}
function endDrag(event){if(!state.drag||event&&event.pointerId!==state.drag.pointerId)return;const nodes=state.drag.starts.map(item=>item.node);state.drag=null;document.body.classList.remove('studio-is-transforming');pushHistory('Move elements');emit('evara:studio-canvas-changed',{reason:'move',nodes})}
...
function endResize(event){if(!state.resize||event&&event.pointerId!==state.resize.pointerId)return;const node=state.resize.node;state.resize=null;document.body.classList.remove('studio-is-transforming');pushHistory('Resize element');emit('evara:studio-canvas-changed',{reason:'resize',nodes:[node]})}
function duplicate(){if(!state.selected.length)return;const copies=state.selected.map(node=>{const copy=renewIds(node.cloneNode(true));copy.style.left=`${px(node.style.left)+16}px`;copy.style.top=`${px(node.style.top)+16}px`;node.after(copy);return copy});clearSelection();copies.forEach((node,index)=>select(node,{add:index>0}));pushHistory('Duplicate elements');emit('evara:studio-canvas-changed',{reason:'duplicate',nodes:copies})}
function removeSelected(){if(!state.selected.length)return;const removed=[...state.selected].filter(node=>!isLocked(node));clearSelection();removed.forEach(node=>node.remove());if(removed.length){pushHistory('Delete elements');emit('evara:studio-canvas-changed',{reason:'delete'})}}
function layer(action){const nodes=state.selected.filter(node=>!isLocked(node));nodes.forEach(node=>{if(action==='front')node.parentElement.append(node);if(action==='back')node.parentElement.prepend(node);if(action==='forward'&&node.nextElementSibling)node.nextElementSibling.after(node);if(action==='backward'&&node.previousElementSibling)node.previousElementSibling.before(node)});if(nodes.length){pushHistory(`Layer ${action}`);positionTransformBox();emit('evara:studio-canvas-changed',{reason:'layer',nodes})}}
function copy(){if(state.selected.length)state.clipboard=state.selected.map(node=>{const clone=node.cloneNode(true);clone.classList.remove('studio-canvas-selected');return clone.outerHTML})}
function paste(){if(!state.clipboard?.length||!state.canvas)return;const nodes=state.clipboard.map(html=>{const template=document.createElement('template');template.innerHTML=html.trim();const node=template.content.firstElementChild;if(!node)return null;renewIds(node);node.style.left=`${px(node.style.left)+20}px`;node.style.top=`${px(node.style.top)+20}px`;state.canvas.append(node);return node}).filter(Boolean);clearSelection();nodes.forEach((node,index)=>select(node,{add:index>0}));pushHistory('Paste elements');emit('evara:studio-canvas-changed',{reason:'paste',nodes})}
function nudge(dx,dy){const nodes=state.selected.filter(node=>!isLocked(node));if(!nodes.length)return;nodes.forEach(node=>{ensurePositioned(node);node.style.left=`${px(node.style.left)+dx}px`;node.style.top=`${px(node.style.top)+dy}px`});positionTransformBox();pushHistory('Nudge elements');emit('evara:studio-canvas-changed',{reason:'nudge',nodes})}
function toggleLock(){if(!state.selected.length)return;state.selected.forEach(node=>{node.dataset.studioLocked=node.dataset.studioLocked==='true'?'false':'true'});renderTransformBox();pushHistory('Toggle element lock');emit('evara:studio-selection',{nodes:state.selected,primary:state.primary});emit('evara:studio-canvas-changed',{reason:'lock',nodes:state.selected})}
function toggleHidden(){if(!state.selected.length)return;state.selected.forEach(node=>{node.dataset.studioHidden=node.dataset.studioHidden==='true'?'false':'true';node.hidden=false});pushHistory('Toggle element visibility');emit('evara:studio-selection',{nodes:state.selected,primary:state.primary});emit('evara:studio-canvas-changed',{reason:'visibility',nodes:state.selected})}
...
function watchCanvas(){state.observer?.disconnect();state.observer=new MutationObserver(()=>{const next=getCanvas();if(next&&next!==state.canvas){state.canvas=next;state.canvas.dataset.studioCanvas='true';getNodes().forEach(nodeId);clearSelection();state.history=[];state.future=[];pushHistory('Canvas reloaded');emit('evara:studio-canvas-ready',{canvas:state.canvas})}});state.observer.observe(document.getElementById('appRoot')||document.body,{childList:true,subtree:true})}

...
function renderProject() {
  return `<section class="studio-overview-grid"><article class="eva-card studio-large-card"><p class="settings-kicker">PROJECT CENTER</p><h2>Studio Core implementation</h2><p>Current focus: Blueprint Engine, draft/publish, live role preview, then Canvas Engine.</p><div class="studio-module-progress"><i style="width:${studioOverallProgress()}%"></i></div><small>Studio Core progress: ${studioOverallProgress()}%</small></article>${STUDIO_WAVES.map((wave) => progressLine(wave, 'wave')).join('')}${STUDIO_CORE_SYSTEMS.map((system) => progressLine(system, 'system')).join('')}</section>`;
}

...
const IS_STUDIO_ROUTE = /(?:website-builder|studio|blueprint|design-system)/i.test(location.pathname);
...
function editableTargets() {
  return Array.from(document.querySelectorAll('main h1,main h2,main h3,main p,main strong,main small,main .btn,main button:not([data-owner-ignore]),main img,main .glass-card,main .eva-card,main [data-builder-slot]'))
    .filter((node) => !node.closest('.owner-editor-panel,.owner-edit-fab-global,.owner-edit-dock,.eva-nav-layer,.evara-loader-fast,.studio-topbar,.studio-dock,.studio-context-toolbar,.studio-sheet,.studio-owner-v2,.studio-canvas-workbench'))
...
function style() {
  ...
  tag.textContent = `
    .owner-edit-dock{position:fixed;right:18px;bottom:calc(var(--eva-page-nav-bottom-clearance,92px) + env(safe-area-inset-bottom,0px));z-index:9997;display:none;align-items:center;gap:10px;padding:7px;border-radius:999px;background:linear-gradient(145deg,rgba(255,255,255,.22),rgba(255,255,255,.08));border:1px solid rgba(255,255,255,.36);box-shadow:0 18px 44px rgba(0,0,0,.22),inset 0 1px 0 rgba(255,255,255,.30);backdrop-filter:blur(26px) saturate(1.35);-webkit-backdrop-filter:blur(26px) saturate(1.35)}
    ...
    .owner-editor-active [data-owner-edit-id],.owner-editor-active [data-builder-slot]{position:relative;outline:1.5px dashed rgba(255,255,255,.54)!important;outline-offset:5px!important;border-radius:12px!important;cursor:pointer!important}.owner-editor-active [data-owner-edit-id]:after,.owner-editor-active [data-builder-slot]:after{content:'';position:absolute;right:6px;top:-10px;z-index:30;width:9px;height:9px;border-radius:999px;background:var(--evara-brand-red,#e30613);border:2px solid rgba(255,255,255,.92);box-shadow:0 5px 16px rgba(227,6,19,.38);pointer-events:none}.owner-selected-live{outline:2px solid rgba(242,23,45,.96)!important;outline-offset:7px!important;box-shadow:0 0 0 7px rgba(242,23,45,.10)!important}
    ...
    .owner-editor-panel{position:fixed;right:18px;top:calc(var(--eva-page-nav-clearance,92px) + 10px);bottom:calc(var(--eva-page-nav-bottom-clearance,92px) + 72px + env(safe-area-inset-bottom,0px));width:min(460px,calc(100vw - 36px));min-height:0;overflow:auto;overscroll-behavior:contain;z-index:9998;display:none;padding:16px;border-radius:28px;background:linear-gradient(145deg,rgba(255,255,255,.28),rgba(10,12,18,.70));border:1px solid rgba(255,255,255,.40);box-shadow:0 24px 64px rgba(0,0,0,.30),inset 0 1px 0 rgba(255,255,255,.30);backdrop-filter:blur(30px) saturate(1.45);-webkit-backdrop-filter:blur(30px) saturate(1.45);color:var(--text-primary,#fff)}
    ...
    @media(max-width:760px){.owner-edit-dock{right:12px;left:12px;bottom:calc(var(--eva-page-nav-bottom-clearance,84px) + env(safe-area-inset-bottom,0px));justify-content:center}.owner-edit-fab-global{flex:1!important}.owner-editor-panel{left:12px;right:12px;top:auto;width:auto;bottom:calc(var(--eva-page-nav-bottom-clearance,84px) + 62px + env(safe-area-inset-bottom,0px));max-height:min(58dvh,560px)}.owner-editor-actions,.owner-editor-grid{grid-template-columns:1fr}}
  `;
...
function ensureUi() {
  ...
  toggle.setAttribute('aria-controls', 'ownerEditorPanel');
  toggle.setAttribute('aria-expanded', 'false');
  const panel = document.createElement('aside');
  panel.id = 'ownerEditorPanel';
  panel.className = 'owner-editor-panel';
  panel.setAttribute('aria-label', 'Owner Editor');
...
function toggleEditor(force) {
  ...
  const toggle = document.querySelector('.owner-edit-toggle');
...
function blockMarkup(type, index) {
  if (type === 'image') return `<img alt="New image" src="/assets/brand/evaraos-app-icon.png" style="width:100%;max-height:240px;object-fit:contain;border-radius:22px;background:rgba(255,255,255,.08);padding:18px">`;
...
function apply() {
  if (IS_STUDIO_ROUTE) {
    document.documentElement.classList.remove('owner-editor-active');
    document.querySelector('.owner-edit-dock')?.remove();
    document.querySelector('.owner-editor-panel')?.remove();
    return;
  }

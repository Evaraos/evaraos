(() => {
  ...
  let logo = window.EVARAOS_CURRENT_LOGO_DATA || FALLBACK;
  ...
  function img() { return `<img class="app-icon-logo-img" src="${logo}" alt="" aria-hidden="true">`; }
  ...
  function sync(id = selectedId()) {
    ...
    document.querySelectorAll('link[rel="icon"], link[rel="shortcut icon"], link[rel="apple-touch-icon"]').forEach(link => { link.href = logo; });
  }
  ...
  function render() { const grid = document.querySelector('[data-app-icon-grid]'); if (!grid) return; const current = selectedId(); grid.innerHTML = backgrounds.map((id, index) => `<button class="app-icon-choice ${id === current ? 'is-active' : ''}" type="button" data-app-icon-choice="${id}" aria-label="${labels[index]}" aria-pressed="${id === current}"><span class="app-icon-preview has-current-logo app-icon-preview--${id}">${img()}</span><span class="app-icon-copy"><strong>${labels[index]}</strong></span></button>`).join(''); sync(current); }
  function bind() { const grid = document.querySelector('[data-app-icon-grid]'); if (grid && grid.dataset.bound !== 'true') { grid.dataset.bound = 'true'; grid.addEventListener('click', event => { const button = event.target.closest('[data-app-icon-choice]'); if (!button) return; const id = button.dataset.appIconChoice; try { localStorage.setItem(KEY, id); } catch {} grid.querySelectorAll('[data-app-icon-choice]').forEach(item => { const active = item.dataset.appIconChoice === id; item.classList.toggle('is-active', active); item.setAttribute('aria-pressed', String(active)); }); sync(id); toast(labels[backgrounds.indexOf(id)] || 'Icon selected'); }); } document.querySelectorAll('[data-apply-selected-icon]').forEach(button => { if (button.dataset.bound === 'true') return; button.dataset.bound = 'true'; button.addEventListener('click', () => toast('Icon saved')); }); document.querySelectorAll('[data-restore-original-icon]').forEach(button => { if (button.dataset.bound === 'true') return; button.dataset.bound = 'true'; button.addEventListener('click', () => { try { localStorage.removeItem(KEY); } catch {} render(); toast('Original icon restored'); }); }); }
  function loadExactLogo() { return new Promise(resolve => { if (window.EVARAOS_CURRENT_LOGO_DATA) { logo = window.EVARAOS_CURRENT_LOGO_DATA; resolve(); return; } const script = document.createElement('script'); script.src = '/assets/js/brand/current-logo-data.js?v=official-png-1'; script.onload = () => { logo = window.EVARAOS_CURRENT_LOGO_DATA || FALLBACK; resolve(); }; script.onerror = resolve; document.head.appendChild(script); }); }
  async function boot() { await loadExactLogo(); render(); bind(); }

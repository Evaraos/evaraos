...
function upsertLink(rel,href,attrs){let link=document.querySelector('link[rel="'+rel+'"]');if(!link){link=document.createElement('link');link.rel=rel;document.head.appendChild(link)}link.href=href+(href.startsWith('data:')?'':'?v=evaraos-icon-system-28');Object.entries(attrs||{}).forEach(([k,v])=>link.setAttribute(k,v))}
function applyInstallManifest(icon){const src=iconSrc(icon);const manifest={name:'Evaraos Inc',short_name:'Evaraos',description:'Subsidiaries Allocation SaaS operating system.',id:'/',start_url:'/?icon='+encodeURIComponent(icon.id),scope:'/',display:'standalone',background_color:themeColor(icon),theme_color:themeColor(icon),icons:[{src,sizes:'any',type:src.startsWith('data:')?'image/svg+xml':'image/svg+xml',purpose:'any maskable'}]};const href='data:application/manifest+json;charset=utf-8,'+encodeURIComponent(JSON.stringify(manifest));upsertLink('manifest',href,{})}
function applyIcon(id){const icon=EVARAOS_APP_ICONS[id]||EVARAOS_APP_ICONS.primaryRed;const src=iconSrc(icon);document.documentElement.dataset.evaraosAppIcon=icon.id;upsertLink('icon',src,{type:'image/svg+xml'});upsertLink('shortcut icon',src,{type:'image/svg+xml'});upsertLink('apple-touch-icon',src,{});applyInstallManifest(icon);const themeMeta=document.querySelector('meta[name="theme-color"]');if(themeMeta)themeMeta.setAttribute('content',themeColor(icon));document.querySelectorAll('[data-evaraos-brand-icon]').forEach(node=>{if(node.tagName==='IMG')node.setAttribute('src',src);node.style.setProperty('--evaraos-brand-icon',"url('"+src+"')")});window.dispatchEvent(new CustomEvent('evaraos:app-icon-change',{detail:icon}));return icon}
...
function hydrateIconPicker(){document.querySelectorAll('[data-app-icon-choice]').forEach(button=>{button.addEventListener('click',()=>{button.classList.remove('is-choosing');void button.offsetWidth;button.classList.add('is-choosing');const selected=saveIcon(button.dataset.appIconChoice);document.querySelectorAll('[data-app-icon-choice]').forEach(choice=>{const active=choice.dataset.appIconChoice===selected.id;choice.classList.toggle('is-active',active);choice.setAttribute('aria-pressed',String(active))});const status=document.getElementById('appIconSaveStatus');if(status){status.textContent='Autosaved: '+selected.label+' selected. Future Android installs and iPhone re-adds will use the selected icon when supported by the browser.';status.hidden=false;status.classList.remove('is-visible');requestAnimationFrame(()=>status.classList.add('is-visible'))}showAutoSave(selected);setTimeout(()=>button.classList.remove('is-choosing'),720)})})}

...
const ownerRoles = new Set(['platform_admin', 'owner', 'super_admin', 'admin']);
...
function verifiedRole() {
  const sessionRole = String(window.EvaraRouteSession?.role || '').trim().toLowerCase();
  if (window.EvaraRouteSession?.authenticated && sessionRole) return sessionRole;
  try {
...
function maybeLoadOwnerEditor(role = '') {
  ...
  if (ownerEditorRequested || isStudioRoute || !ownerRoles.has(normalized)) return;
  ownerEditorRequested = true;
  runWhenIdle(() => Promise.all([
    safeImport('./owner-editor.js?v=8-owner-authority'),
    safeImport('./owner-publish-runtime-v2.js?v=2')
  ]).catch(() => {}));
}
...
function loadNavEnhancements() {
  ...
  Promise.all([
    safeImport('./nav/nav-bottom.js?v=nav-v61-role-authority'),
    safeImport('./nav/nav-drawer-close.js?v=nav-v61-role-authority')
  ]).catch(() => {});
  runWhenIdle(() => Promise.all([
    safeImport('./nav/pull-to-refresh.js?v=nav-v61-role-authority'),
    safeImport('./nav/avatar-sync.js?v=nav-v61-role-authority'),
...
function loadPrivateRuntime() {
  ...
  Promise.all([
    safeImport('./nav/nav-role-lockdown-v2.js?v=2'),
...
window.addEventListener('evara:session-ready', (event) => {
  maybeLoadOwnerEditor(event.detail?.role || verifiedRole());
  loadPrivateRuntime();
});

maybeLoadOwnerEditor(verifiedRole());

...
check("drawer-origin",menuCss.includes("inset:0 auto 0 0")&&stabilityCss.includes("translate3d(-101%,0,0)"),"drawer must be full-height and enter from the left");
check("drawer-open",stabilityCss.includes("body.nav-menu-open .eva-menu-panel")&&stabilityCss.includes("translate3d(0,0,0)"),"open state must use one transform-only path");
...
check("selected-menu-only",cleanControls.includes('[data-active="true"]')&&stabilityCss.includes(".eva-menu-control:not([data-active=\"true\"]):not(.is-pressed)"),"menu rows must be transparent except selected or pressed states");
check("selected-bottom-only",bottomCss.includes('.eva-bottom-link.is-active')&&stabilityCss.includes('.eva-bottom-link:not(.is-active):not([aria-current="page"]):not(.is-pressed)'),"bottom-nav links must be transparent except the current page");
...
check("seven-text-stops",componentsCss.includes("@property --adaptive-text-c6")&&adaptiveJs.includes("TEXT_X=[.02,.18,.34,.5,.66,.82,.98]"),"text gradients must use seven context-aware sample points");
check("context-aware-text",adaptiveJs.includes("backgroundAt(element,x,y,includeSurface=true)")&&adaptiveJs.includes("getComputedStyle"),"text contrast must account for ancestor backgrounds and glass");
check("cached-pixels",adaptiveJs.includes("pixels=x.getImageData"),"adaptive text must sample from a cached pixel buffer");
...
console.log(`Interface audit passed: ${javascriptFiles.length} JavaScript files, ${htmlFiles.length} HTML pages and 17 architecture checks.`);

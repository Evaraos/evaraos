...
for(const file of walk(root)){
  ...
  if(!/\/assets\/css\/nav\.css/.test(html))html=html.replace(/<\/head>/i,'  <link rel="stylesheet" href="/assets/css/nav.css?v=nav-v17" />\n</head>');
  else html=html.replace(/\/assets\/css\/nav\.css\?v=[^"']+/g,'/assets/css/nav.css?v=nav-v17');
  ...
  html=html.replace(/\/assets\/js\/theme-boot\.js\?v=[^"']+/g,'/assets/js/theme-boot.js?v=54');
  html=html.replace(/\/assets\/js\/theme\.js\?v=[^"']+/g,'/assets/js/theme.js?v=54');
  html=html.replace(/\/assets\/css\/theme\.css\?v=[^"']+/g,'/assets/css/theme.css?v=56');
  if(html!==before){fs.writeFileSync(file,html,"utf8");changed.push(path.relative(root,file).replace(/\\/g,"/"))}
}
console.log(`Updated ${changed.length} HTML page(s) for current universal navigation and theme runtime.`);
